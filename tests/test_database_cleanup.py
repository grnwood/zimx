...
def test_delete_folder_cleans_all_subpages(test_db):
    ...
    # /Folder/
    #   Folder.txt
    #   SubPage1/SubPage1.txt
    #   SubPage2/SubPage2.txt
    #   Deep/Nested/Nested.txt
    
    pages = [
        "/Folder/Folder.txt",
        "/Folder/SubPage1/SubPage1.txt",
        "/Folder/SubPage2/SubPage2.txt",
        "/Folder/Deep/Nested/Nested.txt",
    ]
    ...
    assert remaining_pages[0]["path"] == "/Other/Other.txt"
    
    remaining_tasks = config.fetch_tasks()
    assert len(remaining_tasks) == 1
    assert remaining_tasks[0]["id"] == "/Other/Other.txt:99"
...
def test_delete_preserves_links_to_deleted_pages(test_db):
    ...
    config.update_page_index(
        path="/PageA/PageA.txt",
        title="Page A",
        tags=[],
        links=["/PageB/PageB.txt"],
        tasks=[],
    )
    
    config.update_page_index(
        path="/PageB/PageB.txt",
        title="Page B",
        tags=[],
        links=["/PageA/PageA.txt"],
        tasks=[],
    )
    
    # Delete PageB
    config.delete_page_index("/PageB/PageB.txt")
