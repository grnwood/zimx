...
def test_rag_retrieve_pdf(tmp_path: Path) -> None:
    ...
    vault_root.mkdir()
    marker_page = vault_root / "PersonalNotes.txt"
    ...
    try:
        ...
        rag.index_text("/PersonalNotes.txt", "page marker content", kind="page")
        rag.index_text("/PersonalNotes.txt", attachment_text, kind="attachment", attachment=pdf_dest.name)
        ...
        general = rag.query("restaurant", page_refs=["/PersonalNotes.txt"], limit=4)
