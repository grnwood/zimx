...
def _add_business_days(base: date, days: int) -> date:
    if days == 0:
        # If landing on weekend, move forward to Monday
...
def parse_human_date(text: str, today: date, weekdays_only: bool) -> Optional[date]:
    ...
    try:
        ...
        return _apply_weekday_only(iso, weekdays_only)
    except ValueError:
        pass
    ...
    if next_match:
        ...
        if token in WEEKDAY_MAP:
            return _apply_weekday_only(_next_weekday(today, WEEKDAY_MAP[token]), weekdays_only, prefer_forward=True)

    # this <weekday> (same week)
...
class DateInsertDialog(QDialog):
    ...
    def __init__(self, parent=None, anchor_pos=None) -> None:
        ...
        self.date_edit.setPlaceholderText("YYYY-mm-dd or 'next monday', 'in 2 days'…")
        ...
        hint = QLabel("Examples: today, tomorrow, next week, next monday, in 3 days, +2")
    ...
    def _on_calendar_changed(self) -> None:
        ...
        picked = _apply_weekday_only(picked, self.weekdays_only.isChecked())
